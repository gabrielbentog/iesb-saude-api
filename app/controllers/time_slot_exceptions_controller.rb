class TimeSlotExceptionsController < ApplicationController
  ...
  def index
    ...
    render json: @time_slot_exceptions
  end

  # GET /time_slot_exceptions/1
  def show
    render json: @time_slot_exception
  end
  ...
  def create
    ...
    if @time_slot_exception.save
      render json: @time_slot_exception, status: :created, location: @time_slot_exception
    else
      render json: @time_slot_exception.errors, status: :unprocessable_entity
    end
  end
  ...
  private
    ...
    def time_slot_exception_params
      params.expect(time_slot_exception: [ :time_slot_id, :date, :start_time, :end_time, :reason ])
    end
end
